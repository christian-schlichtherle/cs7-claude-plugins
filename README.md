...
| `execute` | "execute the plan", "launch the plan", "resume the run", "relaunch after the blocked report" | Status-aware launch of the plan's own Handoff command as a fresh `claude --bg` process, double-launch guard, attach/logs/stop handles, a live checklist of the run |
...
for you to edit, give more rounds, or drop. Then start the execution phase:
...
claude.ai or your phone, while the session you launched it from keeps a checklist of
its tasks; the same command resumes an interrupted or blocked run.
