...
| `executor` | phase 1, first draft | Model, effort and permission mode for phase 2, in the spellings the pre-flight compares: `claude-opus-5`, `high`, `auto`. The model is the literal ID step 5 resolved the alias to, never the alias. |
...
| `review_rounds` | phase 1, first draft | The review loop's round budget, from the interview that opens step 5 — how many conclusive rounds step 7 may spend before it stops and puts both positions to the user. A reopen reuses it; a plan written before 0.12.0 has none and is asked for it. |
